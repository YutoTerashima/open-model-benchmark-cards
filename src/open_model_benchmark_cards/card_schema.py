...
Purpose: Schema validation for experiment and benchmark-card records.
...
MODULE_PURPOSE = 'Schema validation for experiment and benchmark-card records.'
...
def validate_card(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def required_fields(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = validate_card(materialized)
...
def schema_errors(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
