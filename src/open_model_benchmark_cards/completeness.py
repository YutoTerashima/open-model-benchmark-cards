...
Purpose: Card completeness and reproducibility scoring.
...
MODULE_PURPOSE = 'Card completeness and reproducibility scoring.'
...
def completeness_score(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def artifact_score(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = completeness_score(materialized)
...
def limitation_score(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
