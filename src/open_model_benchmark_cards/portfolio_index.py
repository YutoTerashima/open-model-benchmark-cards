...
"""Project-specific research helpers for Open Model Benchmark Cards.

Purpose: Portfolio-wide card index generation from sibling experiment indexes.
...
MODULE_PURPOSE = 'Portfolio-wide card index generation from sibling experiment indexes.'
...
def load_portfolio_indexes(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def portfolio_table(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = load_portfolio_indexes(materialized)
...
def write_card_index(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
